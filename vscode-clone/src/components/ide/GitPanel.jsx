...
export default function GitPanel() {
  ...
  const handleSync = async () => {
    if (!githubUser) return toast.error("Sign in first!");
    setLoading(true);
    const token = localStorage.getItem('github_token');
    const res = await window.electronAPI.gitPush(rootPath, token);
    if(res.success) toast.success("Synced 🚀");
    else toast.error("Sync failed");
    setLoading(false);
  };

  const handlePublishConfirm = async ({ repoName, isPrivate, selectedFiles }) => {
     const token = localStorage.getItem('github_token');
     setLoading(true);
     const res = await window.electronAPI.gitPublish({ cwd: rootPath, token, repoName, isPrivate, files: selectedFiles });
     if(res.success) { toast.success("Published!"); setHasRemote(true); refreshStatus(); }
      ...
      <div className="p-3 border-b border-[#3c3c3c]">
         ...
         <div className="flex gap-0">
             ...
             <DropdownMenu>
                ...
                <DropdownMenuContent className="bg-[#252526] border-[#454545] text-white"><DropdownMenuItem onClick={() => handleCommitAction()}>Commit</DropdownMenuItem></DropdownMenuContent>
             </DropdownMenu>
         </div>

         {hasRemote ? (
             <Button onClick={handleSync} className="w-full mt-2 bg-[#2d2d2d] hover:bg-[#3c3c3c] text-xs h-7"><RefreshCw size={12} className="mr-2"/> Sync Changes</Button>
         ) : (
